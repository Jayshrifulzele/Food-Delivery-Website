...
let total = Math.floor(subtotal + deliveryFee + taxes);
    ...
    return (
        <div className="bg-slate-200 w-full min-h-screen">
            ...
            {!input?
           <div className="flex flex-wrap justify-center items-center gap-5 w-full py-10">
                {categories.map((item) => {
                    return ( 
                    <div key={item.name}
                     className="w-[140px] h-[150px] bg-white flex flex-col items-start gap-5 p-5 justify-start text-[20px] font-semibold text-gray-600 rounded-lg shadow-xl hover:bg-green-200 cursor-pointer transition-all duration-200" onClick={()=>filter(item.name)}>
                        {item.icon}
                        {item.name}
                ...
                <div className="w-full flex flex-wrap gap-5 px-5 justify-center items-center py-12">
            ...
            />
                ...
                <div className={`w-full md:w-[40vw] h-[100%] fixed top-0 right-0 bg-white shadow-xl p-6 transition-all  duration-500 flex flex-col items-center overflow-auto ${showCart?"translate-x-0":"translate-x-full"}` }>
                    ...
                    <RxCross2 className=" w-[30px] h-[30px] text-green-400 text-[118px] font-semibold cursor-pointer hover:text-gray-600" onClick={()=>setShowCart(false)} />
                    </header>
                    {items.length>0?<>
...
<button
  className="w-[80%] p-3 bg-green-500 rounded-lg text-white hover:bg-green-600 transition-all"
  onClick={() => {
    setShowCart(false);
    navigate("/checkout");
  }}
